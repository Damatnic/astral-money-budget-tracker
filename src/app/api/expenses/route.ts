...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!validationResult.isValid) {
      ...
      await AuditLogger.logSecurityEvent('VALIDATION_FAILED', {
        endpoint: '/api/expenses',
        method: 'POST',
    ...
    if (amount > user.balance + 10000) { // Allow some overdraft
      await AuditLogger.logSecurityEvent('SUSPICIOUS_TRANSACTION', {
        userId: user.id,
        amount,
    ...
    await AuditLogger.logDataAccessEvent('expense_created', {
      userId: user.id,
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    await AuditLogger.logDataAccessEvent('expense_updated', {
      userId: user.id,
      transactionId: id,
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    await AuditLogger.logDataAccessEvent('expense_deleted', {
      userId: user.id,
      transactionId: id,
